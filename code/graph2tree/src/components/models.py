...
class Graph_Module(nn.Module):
	def __init__(self, indim, hiddim, outdim, dropout=0.4):
		...
		self.graph = clones(GCN(indim, hiddim, self.d_k, dropout), 6)
	...
	def forward(self, graph_nodes, graph):
		...
		if not bool(graph.numel()):
			...
			adj_list = [adj,adj,adj,adj,adj,adj]
		else:
			adj = graph.float()
			# print(adj.shape, "adj shape")
			adj_list = [adj[:,1,:],adj[:,1,:],adj[:,4,:],adj[:,4,:],adj[:,5,:],adj[:,5,:]]
		#print(adj)
