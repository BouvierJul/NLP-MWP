...
def train_tree(config, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_value_batch, group_batch, generate_nums,
	...
	if config.embedding == 'bert' or config.embedding == 'roberta':
		...
		# print(contextual_input, "contextual inpy")
		# print(len(contextual_input), "contextual inpy")
		input_seq1, input_len1, token_ids, index_retrieve, pad_word = embedding(contextual_input)
		# print(num_pos)
		# print(input_len1)
		...
		batch_graph = get_single_batch_graph(token_ids.cpu().tolist(), input_len1, new_group_batch, num_value_batch, num_pos, index_retrieve, pad_word)
		batch_graph = torch.LongTensor(batch_graph)
...
def evaluate_tree(config, input_batch, input_length, generate_nums, embedding, encoder, predict, generate, merge, input_lang, output_lang, num_value, num_pos, batch_graph, group_example, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
	...
	if config.embedding == 'bert' or config.embedding == 'roberta':
		...
		try:
			...
			pdb.set_trace()
		batch_graph = get_single_example_graph(token_ids.cpu().tolist()[0], input_len1[0], new_group_example, num_value, num_pos, index_retrieve[0], pad_word[0])
		batch_graph = torch.LongTensor(batch_graph)
