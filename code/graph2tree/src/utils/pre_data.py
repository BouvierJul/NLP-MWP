...
def get_dependency_graph(max_len, input_word_seq, idx):
	# nlp = spacy.load("en_core_web_sm")
	doc = NLP__(' '.join(input_word_seq))
	# print(doc[-1])
	# print(len(doc))
	graph = np.zeros((max_len, max_len))
	childs = {}
	for j in idx:
		token = doc[j]
		childs[token.text] = [j]
...
# def get_dependency_graph(max_len, input_word_seq, idx):
# 	# nlp = spacy.load("en_core_web_sm")
# 	dp = STDP__.dependency_parse(' '.join(input_word_seq))
# 	# print(doc[-1])
# 	# print(len(doc))
# 	graph = np.zeros((max_len, max_len))
# 	childs = {}
# 	# print(len(dependency.nodes))
# 	for k, word in enumerate(dp):
# 		typ, j, i = word
# 		if typ=='ROOT':
# 			add = k
# 		i= add+i-1
...
def get_single_batch_graph(input_batch, input_length,group,num_value,num_pos, index_retrieve=None, pad_word=None):
	# pdb.set_trace()
	batch_graph = []
	...
	for i in range(len(input_length)):
		...
		# print(input_batch_t)
		# print(input_batch_t.index2word, "index")
		if index_retrieve:
				indx = index_retrieve[i]
		if pad_word:
				input_word_seq = pad_word[i]
		sentence_length = input_length[i]
		quantity_cell_list = group[i]
		num_list = num_value[i]
		id_num_list = num_pos[i]
		if index_retrieve:
				graph_depend = get_dependency_graph(max_len, input_word_seq, indx)
		...
		# if index_retrieve:
				# print(graph_depend.shape, graph_greater.shape)
		if index_retrieve:
			graph_total = [graph_newc.tolist(),graph_greater.tolist(),graph_lower.tolist(),graph_quanbet.tolist(),graph_attbet.tolist(), graph_depend.tolist()]
		else:
...
def get_single_example_graph(input_batch, input_length,group,num_value,num_pos, index_retrieve=None, pad_word=None):
	batch_graph = []
